...
SELECT a.first_name as 'First name', b.title as Title, b.grade as Grade
FROM students a JOIN papers b
...
SELECT a.first_name as 'First Name', COALESCE (b.title, 'MISSING') as Title, COALESCE(b.grade, 0) AS Grade
FROM students a LEFT JOIN papers b
ON a.id = b.student_id;

# 4: First name sorted by grade average
SELECT a.first_name as 'First name', ROUND(COALESCE(AVG(b.grade), 0),1) AS Average
FROM students a LEFT JOIN papers b
ON a.id = b.student_id
...
FROM students a 
LEFT JOIN papers b ON a.id = b.student_id
GROUP BY a.first_name
ORDER BY Average DESC;